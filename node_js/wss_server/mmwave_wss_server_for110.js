...
function parser_frame_number(raw) {
  //console.log("raw[1]:%d, raw[2]:%d ",raw[1]&0x7f, raw[2]&0x7f);
  var sum =0;
  if(raw[0] == 0x08){
...
function analysis_frame_and_record(old, receive_frame) {
  ...
  if(old<receive_frame){
	if((old+1) != receive_frame){
		...
		count += loss;
		console.log("missing frame range %d - %d, loss: %d", old+1 , receive_frame-1, loss);	
	}
	old_frame_num = receive_frame;
  }
  else{
	console.log("[[ old > receive_frame ]]");
  }
}
...
function timerFun(){
  ...
  console.log("total:%d, miss:%d",frame_num.toString(),count.toString() );
  console.log("============================================================");
}